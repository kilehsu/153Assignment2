"""
Training script for ChoraleTransformer V3: Chord-token encoding.
Loads Bach chorales, builds chord-state tokenizer, trains the model, and saves checkpoints.
"""
...
def main():
    ...
    sequences_cache = checkpoint_dir / "v3_sequences_cache.pkl"

    # ========== Step 1-3: Load chorales and encode (with cache) ==========
    if sequences_cache.exists() and (checkpoint_dir / "v3_tokenizer.json").exists():
        ...
        print("\nBuilding chord-state tokenizer...")
        tokenizer = BachTokenizerV3()
        tokenizer.build_vocab(chorales)
        print(f"Vocabulary size: {tokenizer.vocab_size}")
        tokenizer.save(str(checkpoint_dir / "v3_tokenizer.json"))
    ...
    train_dataset = ChoraleDataset(train_sequences, context_len=256, stride=8)
    val_dataset   = ChoraleDataset(val_sequences,   context_len=256, stride=8)
    ...
    best_model_path = checkpoint_dir / "v3_transformer_best.pt"
    ...
    print("\nSaving models...")
    final_model_path = checkpoint_dir / "v3_transformer_final.pt"
    ...
    losses_path = checkpoint_dir / "v3_losses.json"
    ...
    try:
        ...
        plt.xlabel("Epoch")
        plt.ylabel("Loss")
        plt.title("Training Curves (V3: Chord-Token Encoding)")
        plt.legend()
        plt.grid(True, alpha=0.3)

        plot_path = images_dir / "09c_training_curves_v3.png"
